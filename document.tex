...
\newcommand{\degree}{Physics with Specialization in Medical Instrumentation}
\newcommand{\degreeAbbrv}{BS PHY-MED}
...
\newcommand{\documentAdviser}{Edgar A. Vallar}
...
\newcommand{\numberOfAuthors}{1} % adapt the number of names below accordingly and sort the sunames in alphabetically ascending manner, like in the following example

\defineAuthor{surname1}{Domingo}
\defineAuthor{firstname1}{John Ericson O.}
...
\newcommand{\documentTitle}{Urban Heat Island} % put tilde (~) between words to indicate non-breaking adjacent words
...
\nocite{*} 
\begin{SingleSpace}
{\printbibliography} %removed the /small
%	\vfill
